...
//función actualizarMontoPago
function actualizarMontoPago() {
  const monto = carrito.reduce((acc, item) => acc + item.producto.precio * item.cantidad, 0);
  const montoPago = document.getElementById('monto-pago');
  
  if (montoPago) {
    const span = montoPago.querySelector('span');
    if (span) {
      span.textContent = `$${monto}`;
    }
  }
}
...
function cargarCarrito() {
  ...
  if (carritoGuardado) {
    carrito = carritoGuardado.filter(item => item.producto && typeof item.producto.precio === 'number');
...
function mostrarDetalleCarrito() {
  ...
  const botonComprar = crearElemento('button', { class: 'boton-comprar' }, 'Comprar');
...
function mostrarFormularioCompra() {
  const modalExistente = document.getElementById('modalFormularioCompra');
  if (modalExistente) {
    modalExistente.remove();
  }

  const modalFormulario = crearElemento('div', { id: 'modalFormularioCompra', class: 'modal' });
  const modalContent = crearElemento('div');
  const botonCerrar = crearElemento('a', { href: '#', class: 'cerrar' }, 'X');
  botonCerrar.addEventListener('click', (event) => {
    event.preventDefault();
  ...
  // Crear el formulario
  const formulario = crearElemento('form', { id: 'formularioCompra', class: 'd-flex justify-content-center' });

  // Contenedor principal para los grupos de inputs
  const container = document.createElement("div");
  container.classList.add("container", "d-flex", "flex-column", "gap-3");

  // Agregar campos del formulario
  const usernameGroup = document.createElement("div");
  usernameGroup.classList.add("input-group", "mb-3");
  ...
  usernameGroup.appendChild(usernameSpan);
  usernameGroup.appendChild(usernameInput);
  container.appendChild(usernameGroup);

  // Campo Email
  const emailGroup = document.createElement("div");
  emailGroup.classList.add("input-group", "mb-3");
  ...
  emailSpan.textContent = "@example.com";
  ...
  phoneGroup.classList.add("input-group", "mb-3");

  const phoneSpan = document.createElement("span");
  phoneSpan.classList.add("input-group-text");
  phoneSpan.textContent = "Telefono";

  const phoneInput = document.createElement("input");
  phoneInput.id = "telefono";
  phoneInput.type = "tel";
  phoneInput.placeholder = "11-2334-2287";
  phoneInput.classList.add("form-control");
  phoneInput.required = true;

  
  phoneInput.pattern = "[0-9]{10}"; 
  phoneInput.title = "El número de teléfono debe contener exactamente 10 dígitos."; 
  phoneInput.maxLength = 10; 

  phoneGroup.appendChild(phoneSpan);
  phoneGroup.appendChild(phoneInput);
  container.appendChild(phoneGroup);

  // Campo Lugar
  const placeGroup = document.createElement("div");
  placeGroup.classList.add("input-group", "mb-3");

  const placeSpan = document.createElement("span");
  placeSpan.classList.add("input-group-text");
  placeSpan.textContent = "Lugar";
  ...
  placeGroup.appendChild(placeSpan);
  placeGroup.appendChild(placeInput);
  container.appendChild(placeGroup);

  // Campo Fecha de entrega
  const dateGroup = document.createElement("div");
  dateGroup.classList.add("input-group", "mb-3");

  const dateSpan = document.createElement("span");
  dateSpan.classList.add("input-group-text");
  dateSpan.textContent = "Fecha de entrega";

  const dateInput = document.createElement("input");
  dateInput.id = "FechaDeEntrega";
  dateInput.type = "date";
  dateInput.classList.add("form-control");

  const hoy = new Date();
  const semanaDesdeHoy = new Date(hoy);
  semanaDesdeHoy.setDate(hoy.getDate() + 7);
  dateInput.min = semanaDesdeHoy.toISOString().split('T')[0];
  ...
  paymentGroup.classList.add("input-group", "mb-3");

  const paymentSpan = document.createElement("span");
  paymentSpan.classList.add("input-group-text");
  paymentSpan.textContent = "Métodos de pago";

  const paymentSelect = document.createElement("select");
  paymentSelect.id = "maquinas";
  paymentSelect.classList.add("form-select");

  const defaultOption = document.createElement("option");
  defaultOption.value = "";
  defaultOption.textContent = "Elija el método de pago";

  const efectOption = document.createElement("option");
  efectOption.value = "efectivo";
  efectOption.textContent = "Efectivo";

  const transferOption = document.createElement("option");
  transferOption.value = "transferencia";
  transferOption.textContent = "Transferencia";

  const creditOption = document.createElement("option");
  creditOption.value = "credito";
  creditOption.textContent = "Tarjeta de crédito";

  paymentSelect.appendChild(defaultOption);
  paymentSelect.appendChild(transferOption);
  paymentSelect.appendChild(creditOption);
  paymentSelect.appendChild(efectOption);

  paymentGroup.appendChild(paymentSpan);
  paymentGroup.appendChild(paymentSelect);
  container.appendChild(paymentGroup);

  // Botón de enviar
  const submitGroup = document.createElement("div");
  submitGroup.classList.add("input-group", "d-flex", "justify-content-center", "mt-3");

  const submitButton = document.createElement("input");
  submitButton.type = "submit";
  submitButton.name = "Submit";
  submitButton.classList.add("btn", "btn-danger", "mb-3");
  ...
  alertParagraph.id = "monto-pago";
  alertParagraph.innerHTML = 'Monto total a pagar: <span>$0</span>';
  ...
  modalFormulario.appendChild(modalContent);
  document.querySelector('main').appendChild(modalFormulario);
...
let publicidades = [
  "30% de descuento pagando en efectivo",
  "2x1 en todos los accesorios",
  "Hasta 6 cuotas sin interes en todos los bancos adheridos",
]
